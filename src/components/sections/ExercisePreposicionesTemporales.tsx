...
export function ExercisePreposicionesTemporales() {
  ...
  useEffect(() => {
    const loadExercises = async () => {
      setLoading(true);
      try {
        const data = await getPreposicionesTemporalesCentralAsync("preposiciones-temporales", currentVariant);
        setExercises(data);
      } catch (error) {
        console.error("Failed to load exercises:", error);
        // Fallback to variant 1
        const data = await getPreposicionesTemporalesCentralAsync("preposiciones-temporales", 1);
        setExercises(data);
      } finally {
        setLoading(false);
      }
  ...
  const {
    ...
    recordHintUsed,
    resetSection,
  } = useSectionState("preposiciones-central", exercises.length);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>🔮 II. Ejercicio Central: Preposiciones Temporales</CardTitle>
        <CardDescription>
          Practica las preposiciones temporales en contextos variados
        </CardDescription>
          ...
          <>
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground mt-1">
                        Tipo: {exercise.context.timeUnit} | Caso: {exercise.context.case}
                      </p>
                    </div>
                    <div className="text-4xl">
                      {exercise.context.timeUnit === "hora" && "🕐"}
                      {exercise.context.timeUnit === "dia-semana" && "📅"}
                      {exercise.context.timeUnit === "momento-dia" && "🌅"}
                      {exercise.context.timeUnit === "fecha" && "📆"}
                      {exercise.context.timeUnit === "mes" && "📅"}
                      {exercise.context.timeUnit === "estacion" && "🍂"}
                      {exercise.context.timeUnit === "duracion" && "⏳"}
                      {exercise.context.timeUnit === "secuencia" && "➡️"}
                      {exercise.context.timeUnit === "limite" && "🛑"}
                      {exercise.context.timeUnit === "inicio-periodo" && "🚀"}
                      {exercise.context.timeUnit === "periodo-completo" && "📊"}
                      {exercise.context.timeUnit === "festividad-dia" && "🎉"}
                    </div>
                  </div>
            ...
            {progress.completed && (
              ...
              >
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Has dominado las preposiciones temporales en diferentes contextos.
                </p>
              </div>
            )}
          </>
