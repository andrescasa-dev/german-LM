...
import type { PreposicionModalRelacionalExercise } from "@/types/preposiciones-modales-relaciones";
import {
  showCorrectAnswer,
  showIncorrectAnswer,
  showHintToast,
} from "@/lib/toast-service";
import { getPreposicionesModalesCentralAsync } from "@/lib/workshop-loader";

export function ExercisePreposicionesModales() {
  ...
  useEffect(() => {
    const loadExercises = async () => {
      setLoading(true);
      try {
        const data = await getPreposicionesModalesCentralAsync(
  ...
  const {
    ...
    recordHintUsed,
    resetSection,
  } = useSectionState("preposiciones-modales-central", exercises.length);
  ...
  const handleSubmit = (exercise: PreposicionModalRelacionalExercise) => {
  ...
  const handleHint = (exercise: PreposicionModalRelacionalExercise) => {
  ...
  const getIconForMeaning = (meaning: string) => {
    switch (meaning) {
      case "con (compañía)":
      case "con (verbo fijo)":
        return "👥";
      ...
      case "para (beneficio)":
      case "para (finalidad)":
      case "para (verbo fijo)":
        return "🎁";
      case "contra (oposición)":
        return "⚔️";
      default:
        return "🔗";
    }
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>
          🔮 II. Ejercicio Central: Preposiciones Modales y Relacionales
        </CardTitle>
        <CardDescription>
          Practica las preposiciones modales y relacionales en contextos
          variados
        </CardDescription>
          ...
          <>
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground mt-1">
                        Significado: {exercise.context.meaning} | Caso:{" "}
                        {exercise.context.case}
                      </p>
                    </div>
                    <div className="text-4xl">
                      {getIconForMeaning(exercise.context.meaning)}
                    </div>
                  </div>
            ...
            {progress.completed && (
              ...
              >
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Has dominado las preposiciones modales y relacionales en
